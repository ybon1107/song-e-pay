...
const formatDate = (dateString) => {
  const options = { year: 'numeric', month: 'long', day: 'numeric', locale: 'en-US' }; // 영어 포맷으로 변경
...
<template>
  <div class="container mt-5">
      ...
      <div class="card-body">
        ...
        <ul class="list-group list-group-flush">
          <li class="list-group-item">
            <strong>Accommodation Name:</strong> {{ reservationData.accommodationName }}
          </li>
          <li class="list-group-item">
            <strong>Check-in Date:</strong> {{ formatDate(reservationData.checkInDate) }}
          </li>
          <li class="list-group-item">
            <strong>Check-out Date:</strong> {{ formatDate(reservationData.checkOutDate) }}
          </li>
          <li class="list-group-item">
            <strong>Guest Count:</strong> {{ reservationData.guestCount }} guests
          </li>
          <li class="list-group-item">
            <strong>Total Price:</strong> {{ formatPrice(reservationData.totalPrice) }}
          </li>
        </ul>
        <!-- 결제 유저 정보 추가 -->
        ...
        <ul class="list-group list-group-flush">
          <li class="list-group-item">
            <strong>Name:</strong> {{ user.firstName }} {{ user.lastName }} <!-- 유저 이름 -->
          </li>
          <li class="list-group-item">
            <strong>Email:</strong> {{ user.userId }} <!-- 유저 이메일 -->
          </li>
          <li class="list-group-item">
            <strong>Phone:</strong> {{ user.phoneNo }} <!-- 유저 전화번호 -->
          </li>
        </ul>
      </div>
    </div>
    ...
    <div class="card-footer bg-transparent border-0">
      ...
      <button class="btn btn-primary mt-3" @click="kakaoPay">Pay with Kakao</button>
    </div>
  </div>
</template>
