...
const receivedAmount = computed(() => {
  if (selectedAsset.value === 'Song-E Money') {
    return ((exchangeAmount.value * 1) / exchangeRate.value).toFixed(2);
  } else if (selectedAsset.value === 'Won-E Money') {
    return ((refundAmount.value * 1) / exchangeRate.value).toFixed(2);
  }
});
</script>
    ...
    <div class="card">
      ...
      <template v-if="selectedAsset === 'Song-E Money'">
        <nav class="nav flex-column flex-sm-row">
          <a class="flex-sm-fill text-sm-center nav-link" :class="{ active: activeTab === 'charge' }" @click="activeTab = 'charge'" aria-current="page"> 충전 </a>
          <a class="flex-sm-fill text-sm-center nav-link" :class="{ active: activeTab === 'exchange' }" @click="activeTab = 'exchange'"> 환전 </a>
          <a class="flex-sm-fill text-sm-center nav-link" :class="{ active: activeTab === 'withdraw' }" @click="activeTab = 'withdraw'"> 환불 </a>
        </nav>
      </template>
      ...
      <template v-if="selectedAsset === 'Won-E Money'">
        <nav class="nav flex-column flex-sm-row">
          ...
          <a class="flex-sm-fill text-sm-center nav-link" :class="{ active: activeTab === 'refund' }" @click="activeTab = 'refund'"> 환급 </a>
        </nav>
      </template>
