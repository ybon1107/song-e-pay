  ...
  <div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="input-group" style="width: 60%">
        ...
        <input type="text" class="form-control" placeholder="검색어를 입력하세요" v-model="searchQuery" @keyup.enter="applyTransactionFilters(true)" />
      </div>
      <button class="btn btn-outline-secondary btn-sm mt-2 mt-md-0" type="button" @click="toggleFilterModal" style="padding: 4px 8px; font-size: 0.875rem">
        {{ filters.selectedPeriod }} · {{ filters.selectedType }} ·{{ filters.selectedSort }}
        ...
        <table class="table table-bordered align-middle">
          <thead>
            <tr>
              ...
              <th class="text-center d-none d-md-table-cell" style="width: 50%">거래 내역</th>
          ...
          <tbody>
            <tr v-for="transaction in transactions" :key="transaction.historyNo" @click="openModal(transaction)" style="cursor: pointer">
              ...
              <td class="text-end">
                ...
                <span v-if="['충전', '환전', '환불'].includes(transaction.typeCode)"> USD </span>
                <span v-else-if="['환급', '송금'].includes(transaction.typeCode)"> KRW </span>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <!-- 필터 모달 -->
    <FilterModal :isVisible="isFilterModalVisible" :filters="filters" @closeModal="toggleFilterModal" @applyFilters="handleFilterApply" />

    <!-- 이용내역 상세 모달 -->
    <HistoriesDetailModal :transaction="selectedTransaction" :isVisible="isModalVisible" @updateMemo="handleMemoUpdate" @close="closeModal" />
  </div>
</template>
