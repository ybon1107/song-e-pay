...
const applyTransactionFilters = async () => {
  ...
  const filters = {
    ...
    beginDate: startDate.value ? new Date(startDate.value).toISOString() : null,
  ...
  try {
    const response = await historyApi.applyFilters(filters, pageRequest.value);
...
const formatUnixTimestamp = (unixTimestamp) => {
  return unixTimestamp ? moment(unixTimestamp).format('YYYY-MM-DD hh:mm') : '';
};
  ...
  <div class="container-fluid">
    ...
    <div class="card p-5 mt-3">
      <div class="mb-3">
        ...
        <div class="row">
          <div class="col-md-3 mb-3">
            ...
            <input id="startDate" type="date" v-model="startDate" @click="openDatePicker" class="form-control" />
          </div>
          <div class="col-md-3 mb-3">
            <label for="endDate">끝 날짜</label>
            <input id="endDate" type="date" v-model="endDate" :min="startDate" @click="openDatePicker" class="form-control" />
          </div>
          <div class="col-md-4 d-flex align-items-end mb-3">
            <input type="text" class="form-control" placeholder="상세 내용 검색" v-model="searchQuery" @keyup.enter="applyTransactionFilters" />
          </div>
          <div class="col-md-2 d-flex align-items-end mb-3">
            <button @click="applyTransactionFilters" class="btn btn-success mb-0">Apply</button>
          </div>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-center my-4">
        <button class="btn btn-secondary mx-1" @click="goToPage(pageRequest.page - 1)" :disabled="pageRequest.page === 1">Previous</button>
        <button class="btn btn-secondary mx-1" @click="goToPage(pageRequest.page + 1)" :disabled="pageRequest.page === totalPages">Next</button>
